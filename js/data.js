...
function prepareVisualizationData() {
    ...
    try {
        ...
        visualizationData.radarData = Object.entries(dashboardData.habilidadesGrupo).map(([skill, info]) => ({
            subject: skill,
            value: Number(parseFloat(info.porcentaje_promedio).toFixed(1)),
            fullMark: 100
        }));
        ...
        visualizationData.objetivosMasComunes = dashboardData.objetivos.analisis_objetivos.mas_comunes.map(([objetivo, count]) => ({
            name: objetivo,
            value: count
        }))
        .sort((a, b) => b.value - a.value) // Ordenar por cantidad
        .slice(0, 7); // Tomar solo los 7 primeros para mejor visualización
        visualizationData.objetivosMasComunes.forEach(item => {
            item.value = Number(parseFloat(item.value).toFixed(1));
        });
        
        // 4. Áreas de interés más comunes
        visualizationData.areasMasComunes = dashboardData.objetivos.analisis_areas.mas_comunes.map(([area, count]) => ({
            name: area,
            value: count
        }))
        .sort((a, b) => b.value - a.value) // Ordenar por cantidad
        .slice(0, 7); // Tomar solo los 7 primeros
        ...
        Object.entries(dashboardData.analisisIrt.categorized_items || {}).forEach(([category, items]) => {
            dificultadPorCategoria[category] = items.length;
        });
        ...
        visualizationData.secuenciaData = dashboardData.recomendacionesPedagogicas.secuencia_recomendada.map((fase, index) => ({
            id: index + 1,
            fase: fase.fase,
            duracion: fase.duracion_estimada,
        ...
        visualizationData.factorialComponentsData = dashboardData.analisisFactorial.factors_skills 
            ? Object.entries(dashboardData.analisisFactorial.factors_skills).map(([factor, skills]) => ({
                ...
                variance: dashboardData.analisisFactorial.explained_variance[parseInt(factor.split(' ')[1]) - 1] * 100
            }))
            : [];
