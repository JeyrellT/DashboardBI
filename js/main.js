...
async function renderIndividualView(participantId) {
    try {
        ...
        if (!window.perfilesAnaliticos || !window.perfilesAnaliticos.perfiles) {
            console.log('Datos de perfiles no disponibles, cargando...');
            try {
                await loadPerfilesAnaliticos();
            } catch (error) {
                console.error('Error al cargar perfiles:', error);
                return; // La función loadPerfilesAnaliticos ya muestra el error
            }
        }
        
        // Verificar existencia del participante después de cargar datos
        if (!participantId) {
        ...
        const perfilesData = window.perfilesAnaliticos?.perfiles || {};
        const perfil = perfilesData[participantId];
        ...
        // Continuar con el renderizado normal del perfil
        renderParticipantProfile(perfil);
    } catch (error) {
        console.error(`Error al renderizar vista individual para participante ${participantId}:`, error);
        app.DOM.dashboardContent.innerHTML = `
            <div class="error-container">
...
async function loadPerfilesAnaliticos() {
    ...
    while (retries < MAX_RETRIES) {
        try {
            console.log('Intentando cargar perfiles analíticos...');
            ...
            if (app.DOM.dashboardContent) {
                app.DOM.dashboardContent.innerHTML = `
                    <div class="loading-container">
                        <div class="loading-spinner"></div>
                        <p>Cargando datos de perfiles analíticos (intento ${retries + 1}/${MAX_RETRIES})...</p>
                    </div>
                `;
            }
            
            // Añadir parámetro de caché para evitar problemas de caché
            ...
            // Guardar datos y verificar que el participante 2 exista
            window.perfilesAnaliticos = data;
            
            console.log('Perfiles cargados exitosamente. Total perfiles:', 
                        Object.keys(data.perfiles).length);
            
            // Verificar si existe el perfil específico del participante 2
            if (data.perfiles['2']) {
                console.log('El perfil del participante 2 está disponible');
            } else {
                console.warn('El perfil del participante 2 NO está disponible');
            }
            
            return data;
        } catch (error) {
            retries++;
            console.error(`Error cargando perfiles (intento ${retries}/${MAX_RETRIES}):`, error);
            
            if (retries >= MAX_RETRIES) {
                if (app.DOM.dashboardContent) {
                    app.DOM.dashboardContent.innerHTML = `
                        <div class="error-container">
                            <div class="error-title">Error al cargar datos de perfiles</div>
                            <div class="error-message">${error.message}</div>
                            <button class="tab-button primary mt-4" onclick="loadPerfilesAnaliticos()">
                                Reintentar
                            </button>
                        </div>
                    `;
