...
async function loadPerfilesAnaliticos() {
    try {
        ...
        // Verificar si se puede acceder al participante 5 (prueba clave)
        const participante5 = window.perfilesAnaliticos.perfiles["5"];
        console.log('¿Participante 5 encontrado?', participante5 ? "SÍ" : "NO");
        
        if (participante5) {
            console.log('ID interno del participante 5:', participante5.id);
        } else {
            console.warn('El participante 5 no está disponible, IDs disponibles:', 
                Object.keys(window.perfilesAnaliticos.perfiles).slice(0, 5));
        }
